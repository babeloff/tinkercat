...
This reference describes TinkerCat' multi-property and meta-property system: property cardinality, meta-properties, the `PropertyManager` API, and the `PropertyQueryEngine`.
For general API coverage see xref:api.adoc[API Reference].

== Overview

TinkerCat now provides comprehensive support for:
...
TinkerCat supports three cardinality types for vertex properties:
...
TinkerCat automatically maintains indices for faster property lookups:
...
The multi-property and meta-property support in TinkerCat provides powerful capabilities for modeling complex property relationships while maintaining performance and data integrity. The system offers:
...
This implementation brings TinkerCat closer to full Apache TinkerPop compatibility while providing additional features for advanced graph modeling scenarios.
