= Advanced Property Indexing and Querying
:toc: left
:toclevels: 3
:sectanchors:
:icons: font
...
This reference describes TinkerCat' advanced indexing capabilities: composite indices, range query optimization, intelligent query planning, and memory-efficient caching.
For general API coverage see xref:api.adoc[API Reference].
To learn how to speed up specific queries with indexing see xref:../how-to/[How-to guides].
...
=== Integration
Seamless integration with existing TinkerCat APIs and property management.
...
The system maintains full backward compatibility while providing significant performance improvements for complex querying scenarios, making TinkerCat suitable for larger datasets and more demanding applications.
