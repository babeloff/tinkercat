...
This reference documents the TinkerCat API across all supported platforms (JVM, JavaScript, Native).
Each section describes available operations, their parameters and return types, and includes a working code example.
For a guided first use see xref:../tutorials/getting-started.adoc[Getting Started].
For task-specific recipes see the xref:../how-to/[How-to guides].
...
val configuredGraph = TinkerCat.open(mapOf(
...
val productionConfig = mapOf(
    ...
    TinkerCat.GREMLIN_TINKERCAT_CACHE_SIZE to 10000,

    // Batch processing
    TinkerCat.GREMLIN_TINKERCAT_BATCH_SIZE to 1000,

    // Index optimization
    TinkerCat.GREMLIN_TINKERCAT_AUTO_INDEX to true
)

val graph = TinkerCat.open(productionConfig)
...
val project = graph.addVertex("project", "name", "TinkerCat", "status", "Active")
...
graph.createIndex("email", TinkerCat.IndexType.SINGLE_PROPERTY)
...
[source,kotlin]
----
import org.apache.tinkerpop.gremlin.tinkercat.io.graphson.*
...
try {
    val problematicGraph = TinkerCat.open()
...
fun serializeLargeGraph(graph: TinkerCat, batchSize: Int = 1000) {
...
fun serializeGraphBinary(graph: TinkerCat, filename: String) {
    try {
...
fun deserializeGraphBinary(filename: String): TinkerCat {
    val graph = TinkerCat.open()
...
fun serializeGraphToJson(graph: TinkerCat): String {
...
fun deserializeGraphFromJson(jsonString: String): TinkerCat {
    val graph = TinkerCat.open()
...
val concurrentGraph = TinkerCat.open(mapOf(
    TinkerCat.GREMLIN_TINKERCAT_CONCURRENT_ACCESS to true
))
...
val transactionalGraph = TinkerCat.open(mapOf(
    TinkerCat.GREMLIN_TINKERCAT_TRANSACTIONS to true
))
...
fun analyzeGraphPerformance(graph: TinkerCat) {
    // Basic graph metrics
...
fun benchmarkOperations(graph: TinkerCat) {
    val iterations = 1000
...
class GraphOperationService(private val graph: TinkerCat) {
...
class ManagedGraph : AutoCloseable {
    ...
    fun executeInTransaction(operation: (TinkerCat) -> Unit) {
        checkNotClosed()
...
This comprehensive API guide demonstrates the full capabilities of TinkerCat across different platforms and use cases. The examples show:
...
For more detailed information about specific APIs, consult the generated API documentation. The examples in this guide provide a solid foundation for building high-performance graph applications with TinkerCat.
