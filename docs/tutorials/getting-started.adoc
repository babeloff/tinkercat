= Getting Started with TinkerCat
:toc: left
:toclevels: 3
:sectnums:
:icons: font
...
By the end of this tutorial you will have TinkerCat installed, a small graph built in memory, and a basic traversal running successfully.
You do not need prior experience with graph databases — everything you need is introduced as you go.
For complete API details see the xref:../reference/api.adoc[API Reference].
For task-specific instructions such as migrating from the Java implementation see xref:../how-to/migrate-from-java.adoc[How-to guides].
...
Add TinkerCat to your project dependencies:
...
TinkerCat supports the standard TinkerPop graph elements:
...
TinkerCat provides powerful indexing capabilities:
...
graph.createIndex("id", TinkerCat.IndexType.SINGLE_PROPERTY) // Fast lookups
graph.createIndex("created", TinkerCat.IndexType.RANGE) // Range queries
...
val config = mapOf(
    // Basic configuration
    TinkerCat.GREMLIN_TINKERCAT_VERTEX_ID_MANAGER to UUID.randomUUID().toString(),
    TinkerCat.GREMLIN_TINKERCAT_EDGE_ID_MANAGER to UUID.randomUUID().toString(),
...
fun validateGraph(graph: TinkerCat): List<String> {
...
* **Contributing Guide**: Help improve TinkerCat
...
TinkerCat provides a powerful, flexible graph database solution that scales from simple applications to enterprise-grade systems. The multiplatform architecture ensures you can use the same API across different deployment environments while taking advantage of platform-specific optimizations.
