= Liberal Parameters: Why API Methods Accept Broad Input Types
:toc:
:toclevels: 3

== Overview

TinkerCat API methods accept broader parameter types than you might expect and handle type conversion internally rather than requiring callers to cast first.
This page explains *why* that design was chosen — the problem it solves and the tradeoffs it makes.
If you are looking for how to call specific methods, see the xref:../reference/api.adoc[API Reference] instead.
...
=== For Developers Using TinkerCat
...
This architectural improvement transforms TinkerCat from a casting-heavy API to a clean, platform-neutral interface that handles type complexity transparently.
