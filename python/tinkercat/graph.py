"""
High-level Python API for TinkerCat.

This module provides a Pythonic interface to the native TinkerCat library,
...
class TinkerCat:
    """
    Python wrapper for TinkerCat providing a Pythonic API.

    This class provides access to the native Kotlin TinkerCat implementation
    ...
    def _check_not_closed(self):
        ...
        if self._closed:
            raise TinkerCatError("Graph has been closed")
    ...
    def add_vertex(self, label: Optional[str] = None, vertex_id: Optional[str] = None,
        ...
        Raises:
            TinkerCatError: If vertex creation fails
            TinkerCatValidationError: If parameters are invalid
        """
        ...
        try:
            ...
            raise TinkerCatVertexError(f"Failed to add vertex: {e}") from e
    ...
    def add_edge(self, label: str, out_vertex: 'Vertex', in_vertex: 'Vertex',
        ...
        Raises:
            TinkerCatError: If edge creation fails
            TinkerCatValidationError: If parameters are invalid
        """
        self._check_not_closed()
        ...
        try:
            ...
            raise TinkerCatEdgeError(f"Failed to add edge: {e}") from e
    ...
    def __str__(self) -> str:
        ...
        return f"TinkerCat(vertices={self.vertex_count}, edges={self.edge_count})"

    def __repr__(self) -> str:
        """Developer representation of the graph."""
        return f"<TinkerCat at 0x{id(self):x}: {self.vertex_count}V, {self.edge_count}E>"


class Vertex:
    """Represents a vertex in the TinkerCat."""

    def __init__(self, graph: TinkerCat, ptr: int, vertex_id: str,
    ...
    def _graph(self) -> Optional[TinkerCat]:
...
class Edge:
    """Represents an edge in the TinkerCat."""

    def __init__(self, graph: TinkerCat, ptr: int, edge_id: str, label: str,
    ...
    def other_vertex(self, vertex: Vertex) -> Vertex:
        ...
        if vertex == self.out_vertex:
            ...
            raise TinkerCatValidationError("Vertex is not connected to this edge")
