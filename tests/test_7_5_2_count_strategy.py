...
class CountStrategyGraph(MockGraph):
    """Graph that maintains a label map for O(1) count short-circuit."""
    ...
    def add_vertex(self, label="vertex", **props):
        v = super().add_vertex(label, **props)
        self._label_map.add(v)
        return v

    def fast_count(self, label=None) -> int:
        if label is None:
            return self._label_map.count_all()
        return self._label_map.count_label(label)
