...
def _save_json(graph, path):
    data = {
        "vertices": [
            {"id": v.id, "label": v.label, "properties": dict(v._properties)}
...
def _load_json(path):
    ...
    for vd in data["vertices"]:
        v = g.add_vertex(vd["label"], vertex_id=vd["id"], **vd["properties"])
        vertex_map[vd["id"]] = v
...
def test_json_roundtrip_vertex_count(tmp):
    ...
    _save_json(g1, path)
    g2 = _load_json(path)
    assert sum(1 for _ in g2.vertices()) == sum(1 for _ in g1.vertices())
...
def test_json_roundtrip_edge_count(tmp):
    ...
    _save_json(g1, path)
    g2 = _load_json(path)
    assert sum(1 for _ in g2.edges()) == sum(1 for _ in g1.edges())
...
def test_backup_restore_round_trip(tmp):
    ...
    assert sum(1 for _ in g2.vertices()) == 6
