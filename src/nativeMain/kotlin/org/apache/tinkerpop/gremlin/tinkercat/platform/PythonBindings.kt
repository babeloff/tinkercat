...
@CName("tinkercat_create")
fun createTinkerCat(): COpaquePointer? {
    return try {
        StableRef.create(TinkerCat.open()).asCPointer()
    } catch (e: Exception) {
        null
    }
}

@CName("tinkercat_destroy")
fun destroyTinkerCat(graphPtr: COpaquePointer?) {
    try {
        graphPtr?.asStableRef<TinkerCat>()?.dispose()
    } catch (e: Exception) {
        // Ignore errors during cleanup
    }
}

@CName("tinkercat_add_vertex")
fun addVertex(graphPtr: COpaquePointer?, id: CPointer<ByteVar>?): COpaquePointer? {
    return try {
        val graph = graphPtr?.asStableRef<TinkerCat>()?.get() ?: return null
...
@CName("tinkercat_add_vertex_with_properties")
...
@CName("tinkercat_add_edge_with_properties")
...
@CName("tinkercat_destroy_vertex")
...
@CName("tinkercat_get_error_message")
