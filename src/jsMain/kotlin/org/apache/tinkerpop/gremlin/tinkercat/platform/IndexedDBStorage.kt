...
/**
 * IndexedDB-based storage implementation for TinkerCat.
 *
...
class IndexedDBStorage(
    private val databaseName: String = "TinkerCatDB",
    ...
    override fun store(graph: TinkerCat, key: String): Promise<Unit> {
    ...
    override fun load(key: String): Promise<TinkerCat?> {
        return ensureDatabase().then { db ->
            Promise<TinkerCat?> { resolve, reject ->
                try {
    ...
    private fun countVertices(graph: TinkerCat): Int {
        var count = 0
        graph.vertices().forEach { count++ }
        return count
    }

    private fun countEdges(graph: TinkerCat): Int {
        var count = 0
