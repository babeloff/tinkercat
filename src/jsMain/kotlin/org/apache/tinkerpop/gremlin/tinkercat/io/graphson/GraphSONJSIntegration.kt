...
import org.apache.tinkerpop.gremlin.tinkercat.platform.GraphSerializer
...
object GraphSONJSIntegration {
    ...
    /**
     * Serializes a TinkerCat to GraphSON v3.0 format with JavaScript optimization.
     ...
     */
    fun serializeToGraphSON(graph: TinkerCat): dynamic {
    ...
    /**
     * Deserializes a TinkerCat from GraphSON v3.0 format with JavaScript optimization.
     ...
     */
    fun deserializeFromGraphSON(data: dynamic): Promise<TinkerCat> {
     ...
     */
    fun toStorageFormat(graph: TinkerCat): dynamic {
        val result = js("{}")
    ...
    /**
     * Restores a TinkerCat from storage format, supporting both GraphSON v3.0 and legacy formats.
     */
    fun fromStorageFormat(storageData: dynamic): Promise<TinkerCat> {
        return Promise { resolve, reject ->
            try {
     ...
     */
    fun serialize(graph: TinkerCat, useGraphSON: Boolean = true): dynamic {
 ...
 */
fun TinkerCat.toGraphSONJS(): dynamic = GraphSONJSIntegration.serializeToGraphSON(this)

fun TinkerCat.toStorageFormatJS(): dynamic = GraphSONJSIntegration.toStorageFormat(this)
