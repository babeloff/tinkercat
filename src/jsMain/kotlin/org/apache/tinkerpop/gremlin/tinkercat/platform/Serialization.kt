...
import org.apache.tinkerpop.gremlin.tinkercat.util.SafeCasting
import kotlin.js.Promise

/**
 * Clean serialization utilities for TinkerCat in JavaScript environments.
 *
 * This provides a simple, efficient way to serialize and deserialize TinkerCat
...
object GraphSerializer {
    ...
    /**
     * Serialize a TinkerCat to a JavaScript object.
     *
     * @param graph The graph to serialize
     * @return A JavaScript object containing all graph data
     */
    fun serialize(graph: TinkerCat): dynamic {
    ...
    private fun serializeWithGraphSON(graph: TinkerCat): dynamic {
    ...
    private fun serializeWithLegacyFormat(graph: TinkerCat): dynamic {
        val result = js("{}")
     ...
     */
    fun toJson(graph: TinkerCat): String {
     ...
     */
    fun toGraphSON(graph: TinkerCat): String {
     ...
     */
    fun fromGraphSON(graphsonString: String): Promise<TinkerCat> {
        return Promise { resolve, reject ->
            try {
    ...
    private fun deserializeWithGraphSON(data: dynamic): Promise<TinkerCat> {
        return Promise { resolve, reject ->
            try {
    ...
    private fun deserializeWithLegacyFormat(data: dynamic): Promise<TinkerCat> {
        return Promise { resolve, reject ->
            try {
                validateFormat(data)
     ...
     */
    fun fromJson(json: String): Promise<TinkerCat> {
        return Promise { resolve, reject ->
            try {
    ...
    private fun deserializeVertex(graph: TinkerCat, data: dynamic): TinkerVertex {
        val id = deserializeId(data.id)
        val label = data.label.unsafeCast<String>()

        val vertex = graph.addVertex(label) as TinkerVertex

        // Set vertex ID if possible (this would require TinkerCat modification)
    ...
    private fun deserializeEdge(graph: TinkerCat, data: dynamic, vertexMap: Map<String, TinkerVertex>) {
        val id = deserializeId(data.id)
        val label = data.label.unsafeCast<String>()
    ...
    private fun validateFormat(data: dynamic) {
        if (data._type != "TinkerCat") {
            throw StorageException("Invalid graph format: expected TinkerCat, got ${data._type}")
        }
 ...
 */
fun TinkerCat.toJson(): String = GraphSerializer.toJson(this)
fun TinkerCat.serialize(): dynamic = GraphSerializer.serialize(this)

/**
 * Companion functions for deserialization
 */
fun graphFromJson(json: String): Promise<TinkerCat> = GraphSerializer.fromJson(json)
fun graphFromData(data: dynamic): Promise<TinkerCat> = GraphSerializer.deserialize(data)
