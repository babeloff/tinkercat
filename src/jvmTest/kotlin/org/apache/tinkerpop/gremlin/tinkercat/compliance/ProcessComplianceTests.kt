 ...
 *
 * This test suite validates that TinkerCat implements basic graph traversal and processing operations
 * in compliance with Apache TinkerPop specifications. These tests focus on the core graph operations
 * that are currently implemented in TinkerCat rather than full Gremlin traversal syntax.
 *
 * PROVENANCE:
 ...
 * - Source: https://github.com/apache/tinkerpop/tree/3.7.2/tinkercat-gremlin/src/test/java
 *
 * ADAPTATIONS:
 * - Converted from JUnit to Kotest StringSpec format
 * - Focused on vertex/edge traversal methods available in current TinkerCat implementation
 * - Adapted to test iterator-based graph processing rather than full Gremlin syntax
 * - Cross-platform compatibility focus
 * - Simplified to match actual API surface of TinkerCat
 *
 * Test Categories:
...
@TinkerPopTestSource(
    ...
    adaptations = "Adapted to focus on TinkerCat's actual traversal API; combined multiple test classes; cross-platform compatibility",
    coverage = 0.8
)
