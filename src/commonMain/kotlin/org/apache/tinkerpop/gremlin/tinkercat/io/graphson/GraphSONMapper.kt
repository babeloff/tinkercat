...
class GraphSONMapper private constructor(
    ...
    /**
     * Serializes a TinkerCat to GraphSON v3.0 format.
     */
    fun writeGraph(graph: TinkerCat): String {
        return writer.writeGraph(graph)
    }

    /**
     * Deserializes a TinkerCat from GraphSON v3.0 format.
     * Creates a new TinkerCat with proper configuration for GraphSON compatibility.
     */
    fun readGraph(graphsonString: String): TinkerCat {
        val config = if (allowNullProperties) {
            mapOf(TinkerCat.GREMLIN_TINKERCAT_ALLOW_NULL_PROPERTY_VALUES to true)
        } else {
            emptyMap()
        }
    ...
    /**
     * Deserializes a TinkerCat from GraphSON v3.0 format into an existing graph.
     */
    fun readGraphInto(graphsonString: String, targetGraph: TinkerCat): TinkerCat {
    ...
    class Builder {
        ...
        /**
         * Configure whether to allow null properties in created TinkerCat instances.
...
object GraphSON {
     ...
     */
    fun graphFromGraphSON(graphsonString: String): TinkerCat {
     ...
     */
    fun graphFromGraphSON(graphsonString: String, idConflictStrategy: IdConflictStrategy): TinkerCat {
