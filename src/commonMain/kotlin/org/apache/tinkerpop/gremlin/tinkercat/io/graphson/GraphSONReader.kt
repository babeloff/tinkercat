...
import org.apache.tinkerpop.gremlin.tinkercat.util.LoggingConfig

/**
 * GraphSON v3.0 reader implementation for deserializing TinkerCat structures.
 *
...
class GraphSONReader {
    ...
    /**
     * Deserializes a TinkerCat from a GraphSON v3.0 JSON string.
     */
    fun readGraph(graphsonString: String): TinkerCat {
        return readGraph(graphsonString, IdConflictStrategy.DEFAULT)
    }

    /**
     * Deserializes a TinkerCat from a GraphSON v3.0 JSON string with specified ID conflict strategy.
     */
    fun readGraph(graphsonString: String, idConflictStrategy: IdConflictStrategy): TinkerCat {
        val graphObject = try {
        ...
        // Configure TinkerCat to support null properties for GraphSON v3.0 compatibility
        val config = mapOf(
    ...
    /**
     * Deserializes GraphSON v3.0 data into an existing TinkerCat.
     */
    fun readGraphInto(graphsonString: String, targetGraph: TinkerCat, idConflictStrategy: IdConflictStrategy): TinkerCat {
        val graphObject = try {
            json.parseToJsonElement(graphsonString).jsonObject
        } catch (e: Exception) {
            throw GraphSONException("Failed to parse GraphSON JSON", e)
    ...
    private fun readVertex(graph: TinkerCat, vertexObject: JsonObject): TinkerVertex {
    ...
    private fun readEdge(graph: TinkerCat, edgeObject: JsonObject, vertexMap: Map<Any, TinkerVertex>): TinkerEdge {
    ...
    private fun createVertexWithConflictResolution(
            ...
            return when (strategy) {
                ...
                IdConflictStrategy.GENERATE_NEW_ID -> {
                    val newId = (graph as TinkerCat).getNextId()
    ...
    private fun createEdgeWithConflictResolution(
        ...
        val graph = outVertex.graph() as TinkerCat
