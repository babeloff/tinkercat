...
import org.apache.tinkerpop.gremlin.tinkercat.io.graphson.GraphSONMapper
import org.apache.tinkerpop.gremlin.tinkercat.io.graphson.GraphSONException
...
/**
 * Comprehensive JVM persistence layer for TinkerCat.
 *
...
class JvmPersistenceLayer(
    private val baseDirectory: String = "./tinkercat-data",
    ...
    private fun saveAsJson(graph: TinkerCat, path: Path): PersistenceMetadata {
    ...
    private fun loadFromJson(path: Path): TinkerCat {
    ...
    private fun saveAsXml(graph: TinkerCat, path: Path): PersistenceMetadata {
    ...
    private fun loadFromYaml(path: Path): TinkerCat {
        val yamlString = Files.readString(path)
        val graphData = convertYamlToMap(yamlString)
        return convertSerializableMapToGraph(graphData)
    }

    private fun saveAsGraphML(graph: TinkerCat, path: Path): PersistenceMetadata {
    ...
    private fun loadFromGraphML(path: Path): TinkerCat {
        // Use JSON internally for GraphML until proper GraphML parsing is implemented
    ...
    private fun saveAsGraphSON(graph: TinkerCat, path: Path): PersistenceMetadata {
    ...
    private fun loadFromGraphSON(path: Path): TinkerCat {
        // Use native GraphSON v3.0 implementation
    ...
    private fun saveAsGryo(graph: TinkerCat, path: Path): PersistenceMetadata {
    ...
    private fun loadFromBinary(path: Path): TinkerCat {
        val data = Files.readAllBytes(path)
        return JvmSerialization.deserializeGraph(data)
    }

    private fun convertGraphToSerializableMap(graph: TinkerCat): Map<String, Any> {
        val graphData = convertGraphToSerializableData(graph)
    ...
    private fun convertGraphToSerializableData(graph: TinkerCat): SerializableGraphData {
    ...
    private fun convertSerializableDataToGraph(data: SerializableGraphData): TinkerCat {
        ...
        data.vertices.forEach { vertexData ->
            ...
            // Create vertex with GraphSON-style properties (include the serialized ID for TinkerCat to consider)
            val creationProperties = mutableMapOf<String, Any?>()

            // Include the serialized ID - let TinkerCat decide whether to use it or generate a new one
            ...
            // Create vertex - TinkerCat will handle ID assignment according to its rules
    ...
    private fun convertSerializableMapToGraph(data: Map<String, Any>): TinkerCat {
        val graph = TinkerCatFactory.create()
        ...
        verticesData.forEach { vertexData ->
            ...
            // Don't add id as a property, let TinkerCat assign its own ID
